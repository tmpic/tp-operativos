...
void iniciar_servidor(char* IP, char* PUERTO, void (*f)(t_result*))//handle_client
{
...
void esperar_cliente(int socket_servidor, void (*f)(t_result*))
{
	...
	if (socket_cliente == -1){
		//perror("ERROR DE ACCEPT:");
		//printf("ERROR DE ACCEPT: %s | NUMERO DE ERRNO: %d\n", strerror(errno), errno);//Ojo con esto, incluye <errno.h>
	}
...
void process_request(int cod_op, t_parameter* parametros) {
	...
	recv(parametros->socket, &cod_op, sizeof(int), MSG_WAITALL);
		...
		case MENSAJE:
			...
			free(resultado->identificador_cliente);
		...
		case MENSAJES:
			...
			free(resultado->mensajes->mensajes);
...
void enviar_mensajes(int cantidadDeMensajes, char* mensajes[], int socket_cliente)
{
	...
	paquete->buffer = malloc(sizeof(t_buffer) * paquete->size);//Reservo memoria para muchos buffers
...
void enviar_mensaje(char* mensaje, int socket_cliente)
{
	...
	int bytes = paquete->buffer->size + 2*sizeof(int32_t);//Uno para el size del mensaje y otro para el C_OPeracion
